...
public class CellPhone {
	...
	public CellPhone(CellNumber number, List<CellNumber> callLog) {
		...
		this.callLog= new ArrayList<>();
		if(callLog!=null) {
			for(CellNumber str:callLog) {		
			this.callLog.add(new CellNumber(str));
	}
		}
	}
	/**
	...
	public List<CellNumber> getCallLog() {
		List<CellNumber> list1 = new ArrayList<CellNumber>();	
		for(CellNumber str:callLog) {		
			list1.add(str);		
	}
		return list1;
	...
	public CellNumber favorite() {
		int max=0;
		CellNumber fav = null;
		for(CellNumber str:this.callLog)
		{	
			int freq= Collections.frequency(this.callLog, str);
			if(freq > max) {
				fav = str;
				max=freq;
			}
